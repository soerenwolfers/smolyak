...
class TestApproximator(unittest.TestCase):
    ...
    def test_dimensionadaptivity(self):
        ...
        decomp = Decomposition(func=func, next_dims=lambda dim: [dim+1] if dim<ndim -1 else [])
        SA = Approximator(decomp)
        SA.expand_adaptive(c_steps=10)
    ...
    def test_notbundled(self):
        ...
        decomp=Decomposition(func=func,n=3,contribution_factor=contribution_factor)
        SA = Approximator(decomp)
...
if __name__ == "__main__":
    ...
    #suite.addTest(TestApproximator('test_dimensionadaptivity'))
